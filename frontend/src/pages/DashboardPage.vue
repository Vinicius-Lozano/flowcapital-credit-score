  ...
  <q-page class="capital-dashboard-page">
    <div class="dashboard-wrapper">
      ...
      <nav class="dash-nav">
        ...
        <div class="nav-title">Painel Open Finance</div>
      ...
      <div v-if="!resultado && !conectando" class="empty-state">
        ...
        <p>Clique em "Sincronizar Banco" para emitir sua avaliação de crédito baseada no fluxo de caixa atual.</p>
        <q-btn
...
const simularConexaoBanco = async () => {
  ...
  try {
    ...
    const resposta = await axios.post(
      `${process.env.API_URL}/api/analisar/`,
      {},
      { headers: { Authorization: `Token ${token()}` } },
    )
    resultado.value = resposta.data
  } catch (erro) {
    if (erro.response?.status === 401) router.push('/login')
